...
export async function registerTrustLayerRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/verify/ghana-card", {
    ...
    const verification = await trustLayer.verifyGhanaCard(
      {
      ...
      },
      {
        ...
        idempotencyKey: `ghana_card_${authUser.userId}_${parsed.data.ghanaCardNumber}`
      }
    );
    ...
    if (verification.status !== "verified") {
      void writeAuditLog({
        ...
        action: "GHANA_CARD_VERIFICATION_PENDING",
        entityType: "USER",
